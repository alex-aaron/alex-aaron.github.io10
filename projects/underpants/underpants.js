...
// this code uses dot notation to create a key of identity and assign a function
...
_.each = function(collection, func){
    ...
    if (Array.isArray(collection)){
        ...
        for (let i = 0; i < collection.length; i++){
            // invoke callback function each element, index, and collection itself
            func(collection[i], i, collection);
        }
    } else { // else it's an object
        for (let key in collection){
            // invoke callback on each value, key, and collection
...
_.every = function(collection, func){
    if (Array.isArray(collection)){
        // determine if func didn't receive a value
        if (func === undefined){
            for (let i = 0; i < collection.length; i++){
                if (!collection[i]){ // determine if the current item is not truthy
                    return false;
                }
            }
        } else { // else it did
